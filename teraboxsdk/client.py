...
class TeraBoxClient(BaseTeraBoxClient[HTTPClient]):
    """Synchronous TeraBox client for fetching share info and downloading files.
    ...
    def __init__(self, timeout: float = 30.0, proxy: str | None = None) -> None:
        ...
        self._http = HTTPClient(timeout=timeout, proxy=proxy)
    ...
    def _get_share_info(self, surl: str, password: str | None = None) -> dict[str, Any]:
    ...
    def get_files(
        ...
        *,
        ...
        # Validate share via API
        self._get_share_info(surl, password)
        ...
        *,
        ...
        if recursive:
            for folder in folders:
                sub = self._list_subfolder(folder.fs_id, folder.path)
                folder_info.subfolders.append(sub)

        return folder_info

    def _list_subfolder(self, fs_id: int, path: str) -> FolderInfo:
        """List contents of a subfolder by fs_id."""
        params = self._get_default_params()
        params.update(
            {
                "dir": path,
        ...
        for sf in subfolders:
            sub = self._list_subfolder(sf.fs_id, sf.path)
            folder.subfolders.append(sub)

        return folder

    def get_download_link(self, file: FileInfo) -> DownloadInfo:
    ...
    def download(
        ...
        *,
        chunk_size: int = 8192,
        progress_callback: downloader.ProgressCallback | None = None,
    ) -> Path:
    ...
    def __enter__(self) -> TeraBoxClient:
        return self
